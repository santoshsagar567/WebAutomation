...
public class RedBus_Test {
	...
	private static final String TARGET_DATE = "2026-08-23";

	// Source and destination cities
	private static final String SOURCE = "Bangalore";
	private static final String DESTINATION = "Kolkata";
	// ──────────────────────────────────────────────────────────────────────────

	public static void main(String[] args) {
		RedBus_Test bot = new RedBus_Test();
		try {
			bot.setUp();
			bot.openRedBus();
	...
	public void enterSource(String city) throws InterruptedException {
		...
		WebElement fromBox = wait.until(ExpectedConditions.elementToBeClickable(By.id("srcinput")));
		fromBox.clear();
	...
	public void enterDestination(String city) throws InterruptedException {
		WebElement toBox = wait.until(ExpectedConditions.elementToBeClickable(By.id("destinput")));
		toBox.clear();
		toBox.sendKeys(city);
	...
	private void pickFirstSuggestion(String city) throws InterruptedException {
		...
		List<WebElement> suggestions = wait
				.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//div[@aria-label='"+city+"']")));
	...
	private void navigateToMonth(int targetMonth, int targetYear) throws InterruptedException {
		...
		for (int attempt = 0; attempt < 24; attempt++) { // max 24 months ahead
			...
			WebElement header = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//p[@class='monthYear___a4142d']")));
			//p[@class='monthYear___a4142d'] //p[@class*=monthYear]
			...
			WebElement nextArrow = driver.findElement(By.xpath("//i[contains(@aria-label,'Next month')]"));
			nextArrow.click();
			Thread.sleep(500);
		}
	...
	private void clickDay(int day) {
		...
		List<WebElement> dayCells = driver.findElements(By.xpath("//ul/div/li"));
		//System.out.println(dayCells);
